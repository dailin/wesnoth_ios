...
extern const char* user_cache_dir;
extern const char* user_doc_dir;
#endif
...
{
#ifdef __IPHONEOS__
    return get_user_config_dir() + "/save_index.gz";
#else
...
{
#ifdef __IPHONEOS__
	const std::string dir_path = get_user_config_dir() + "/saves";
#else
...
void set_preferences_dir(std::string path)
{
    ...
    game_config::preferences_dir = std::string(user_cache_dir);
#else
	const char* home_str = getenv("HOME");
	std::string home = home_str ? home_str : ".";
...
#ifdef __IPHONEOS__
    create_directory_if_missing(std::string(user_doc_dir) + "/saves");
#else
...
#elif defined(__IPHONEOS__)
        user_config_dir = std::string(user_doc_dir);
