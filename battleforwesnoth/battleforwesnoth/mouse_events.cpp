...
namespace events{
    ...
    void mouse_handler::left_mouse_up(int x, int y, const bool browse)
    {
        ...
        undo_ = false;
        if (mouse_handler_base::left_click(x, y, browse)) return;
...
//            gRedraw = true;
                ...
                if (attack_from == selected_hex_) { //no move needed
                    ...
                    if (resources::whiteboard->is_active()) {
                        ...
                        if(get_adj_enemies(attack_from, side) != adj_enemies)
                            return; //ambush, interrupt attack
    ...
    int mouse_handler::show_attack_dialog(const map_location& attacker_loc, const map_location& defender_loc)
    {
        ...
        if (bc_vector.empty())
        {
            ...
            gui::show_dialog(gui(), NULL, "Attack Enemy",
        ...
        int res = 0;
        {
            ...
            res = gui::show_dialog(gui(),NULL,"Attack Enemy",
#ifndef USE_TINY_GUI
