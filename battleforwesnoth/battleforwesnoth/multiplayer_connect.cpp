...
#ifdef USE_TINY_GUI
	label_gold_(parent.video(), "Gold: 100", font::SIZE_SMALL, font::LOBBY_COLOR),
	label_income_(parent.video(), _("Income: Normal"), font::SIZE_SMALL, font::LOBBY_COLOR),
#else
...
void connect::side::add_widgets_to_scrollpane(gui::scrollpane& pane, int pos)
{
	...
	curX += 100;
	pane.add_widget(&combo_color_,  curX, 5 + pos);
	curX = 22 + 100;
...
#else
        label_gold_.set_text("Gold: " + (str_cast(gold_)));
#endif
		changed_ = true;
	}
...
#else
        label_income_.set_text("Income: " + (str_cast(buf.str())));
#endif
		changed_ = true;
	}
}
...
void connect::layout_children(const SDL_Rect& rect)
{
	...
	curX += 100;
	color_title_label_.set_location(curX, top+35);
	curX += 6666;
